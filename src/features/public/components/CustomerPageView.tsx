...
import { THEMES, ACCENTS, FONT_PAIRS } from '@/features/gallery/constants/preferences'
...
export default function CustomerPageView({
  ...
  const heroScale = useTransform(scrollYProgress, [0, 1], preview ? [1, 1] : [1, 1.08])
  const heroOpacity = useTransform(scrollYProgress, [0, 0.7], preview ? [1, 1] : [1, 0])
  const heroY = useTransform(scrollYProgress, [0, 1], preview ? ['0%', '0%'] : ['0%', '20%'])
  ...
  return (
    ...
    >
      ...
      <section ref={heroRef} className="relative h-screen overflow-hidden">
        ...
        <motion.div
          className={`absolute bottom-12 flex items-end gap-5 ${
            prefs.coverPosition === 'center'
              ...
              ? mobileLayout ? 'right-8' : 'right-8 sm:right-12 lg:right-16'
              : mobileLayout ? 'left-8' : 'left-8 sm:left-12 lg:left-16'
          }`}
        ...
        >
          {profile.logo && (
            <div className={`shrink-0 overflow-hidden rounded-full ring-1 ring-white/20 ${mobileLayout ? 'size-14' : 'size-14 sm:size-16'}`}>
              <Image
          ...
          <div className={prefs.coverPosition === 'center' ? 'text-center' : undefined}>
            <p
      ...
      {/* ── GALLERIES ── */}
      <section className={`mx-auto max-w-7xl px-6 pb-24 pt-20 ${mobileLayout ? '' : 'sm:px-10 lg:px-16'}`}>
        <motion.div
        ...
        {publishedGalleries.length === 0 ? (
          ...
          <div className={`grid gap-5 ${mobileLayout ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
      ...
      {(profile.aboutText || profile.whatsapp || profile.instagram) && (
        ...
        >
          <div className={`mx-auto grid max-w-7xl grid-cols-1 gap-12 px-6 py-24 ${mobileLayout ? '' : 'sm:px-10 lg:grid-cols-2 lg:px-16'}`}>
            {profile.aboutText && (
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
